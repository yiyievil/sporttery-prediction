...
def _extract_market_bet(r, key, meta, market):
    """从单场比赛提取 HAD 或 HHAD 主推

    market: 'HAD' 或 'HHAD'
    返回 bet dict 或 None
    """
    md = r.get(market, {})
    if not md.get('dir') or not md.get('odds'):
        return None
    # Ultra 7.10: HAD未开盘时跳过
    if market == 'HAD' and md.get('had_open') is False:
        return None
    m = re.findall(r'(\d+(?:\.\d+)?)%', md.get('p', ''))
    if len(m) != 3:
        return None
    # HAD dir: 胜/平/负, HHAD dir: 让胜/让平/让负 → 统一映射
    raw_dir = md['dir']
    dir_map = {'胜': 0, '平': 1, '负': 2,
               '让胜': 0, '让平': 1, '让负': 2}
...
def extract_had_bets(pred_file):
    """从预测文件提取每场最优主推 (HAD/HHAD 取命中率更高者, Ultra 11.33)

    竞彩规则: 胜平负/让球胜平负均可串关最多8关, 不可同场混玩法。
    本函数对每场比赛比较 HAD 和 HHAD 主推概率, 取命中率更高者参与串关。
    命中率(模型概率)第一优先, EV仅作展示参考 (用户铁律, Ultra 11.23/11.24)。
    """
    ...
    for key, r in data.get('results', {}).items():
        ...
        # Ultra 11.33: 按命中率(prob)选市场, EV仅作标注 (ERR-20260810-009)
        # 铁律: "命中率第一优先, EV仅参考" — 选场方向必须看概率, 不看EV
...
def main():
    ...
    M = len(bets)
    print(f"场次: {M}场 (HAD/HHAD最优主推, 概率=SWOT融合后, SP=官方固定奖金)")
    for b in bets:
        mkt = b.get('market', 'HAD')
        hc = f" 让{b['handicap']}" if 'handicap' in b else ''
        print(f"  {b['key']} {b['home']} vs {b['away']} | [{mkt}]{hc} {b['dir']}@{b['odds']} P={b['prob']:.0%} {b['conf']}")
